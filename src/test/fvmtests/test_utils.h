...
inline void initState(){
    ...
    pathTemp = fs::temp_directory_path() / strprintf("test_firovm_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(100000)));
    boost::filesystem::create_directories(pathTemp);
    const std::string dirFvm = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<FvmState>(new FvmState(dev::u256(0), FvmState::openDB(dirFvm, hashDB, dev::WithExisting::Trust), dirFvm + "/fvmDB", dev::eth::BaseState::Empty));
...
inline dev::Address createFvmAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline FvmTransaction createFvmTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    FvmTransaction txEth;
    if(recipient == dev::Address()){
        txEth = FvmTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = FvmTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<FvmTransaction> txs){
