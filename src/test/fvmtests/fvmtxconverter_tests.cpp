...
void checkResult(bool isCreation, std::vector<FvmTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    FvmTxConverter converter(transaction, NULL);
    ExtractFvmTX fvmTx;
    BOOST_CHECK(converter.extractionFvmTransactions(fvmTx));
    std::vector<FvmTransaction> result = fvmTx.first;
    if(script2 == CScript()){
