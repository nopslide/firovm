...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract){
    initState();
    FvmTransaction txEth = createFvmTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<FvmTransaction> txs(1, txEth);
    auto result = executeBC(txs);
    
    std::vector<dev::Address> addrs = {createFvmAddress(txs[0].getHashWith(), txs[0].getNVout())};
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGasIntrinsic){
    initState();
    FvmTransaction txEth = createFvmTransaction(CODE[0], 0, dev::u256(100), dev::u256(1), HASHTX, dev::Address());
    std::vector<FvmTransaction> txs(1, txEth);
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGas){
    initState();
    FvmTransaction txEth = createFvmTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<FvmTransaction> txs(1, txEth);
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGasIntrinsic_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        FvmTransaction txEth = createFvmTransaction(CODE[0], 0, gasLimit, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createFvmAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGas_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        FvmTransaction txEth = createFvmTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createFvmAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_many){
    ...
    for(size_t i = 0; i < 130; i++){
        FvmTransaction txEth = createFvmTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createFvmAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer){
    initState();
    FvmTransaction txEthCreate = createFvmTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<FvmTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    std::vector<dev::Address> addrs = {createFvmAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout())};
    FvmTransaction txEthCall = createFvmTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, addrs[0]);
    std::vector<FvmTransaction> txsCall(1, txEthCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGasIntrinsic_return_value){
    ...
    std::vector<FvmTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    dev::Address newAddress(createFvmAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    FvmTransaction txEthCall = createFvmTransaction(ParseHex("00"), 1300, dev::u256(1), dev::u256(1), HASHTX, newAddress);
    std::vector<FvmTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGas_return_value){
    initState();
    FvmTransaction txEthCreate = createFvmTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<FvmTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    dev::Address newAddress(createFvmAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    FvmTransaction txEthCall = createFvmTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, newAddress);
    std::vector<FvmTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_many){
    ...
    executeBC(txs);
    std::vector<FvmTransaction> txsCall;
    std::vector<dev::Address> addrs;
    valtype codeCall(ParseHex("00"));
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        FvmTransaction txEthCall = createFvmTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hash, newAddressGen[i], i);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_OutOfGas_transfer_many_return_value){
    ...
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 130; i++){
        FvmTransaction txEth = createFvmTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createFvmAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    ...
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        FvmTransaction txEthCall = createFvmTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hashCall, newAddressGen[i], i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
        ++hashCall;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_suicide){
    initState();
    std::vector<FvmTransaction> txsCreate;
    std::vector<dev::Address> newAddresses;
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 10; i++){
        valtype code = i == 0 ? CODE[0] : CODE[4];
        dev::u256 value = i == 0 ? dev::u256(0) : dev::u256(13);
        FvmTransaction txEthCreate = createFvmTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        txsCreate.push_back(txEthCreate);

        FvmTransaction txEthSendValue;
        if(i != 0){
            txEthSendValue = createFvmTransaction(valtype(), value, GASLIMIT, dev::u256(1), hash, createFvmAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()), i);
            txsCreate.push_back(txEthSendValue);
        }

        newAddresses.push_back(createFvmAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
        ++hash;
    }
    ...
    for(size_t i = 0; i < newAddresses.size() - 1; i++){
        FvmTransaction txEthCall = createFvmTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddresses[i + 1]);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_contract_create_contracts){
    initState();
    FvmTransaction txEthCreate = createFvmTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<FvmTransaction> txs(1, txEthCreate);
    executeBC(txs);
    std::vector<FvmTransaction> txsCall;
    valtype codeCall(ParseHex("3f811b80"));
    dev::Address newAddress(createFvmAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
    std::vector<dev::Address> addrs;
    for(size_t i = 0; i < 20; i++){
        FvmTransaction txEthCall = createFvmTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddress, i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall);
