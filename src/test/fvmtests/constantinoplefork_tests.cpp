...
void genesisLoading(){
    ...
    int forkHeight = COINBASE_MATURITY + 499;
...
//    initState();
    ...
    txs.push_back(createFvmTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createFvmTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createFvmAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<FvmTransaction> txsCall;
    txsCall.push_back(createFvmTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<FvmTransaction> txsMint;
    txsMint.push_back(createFvmTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<FvmTransaction> txsbalance;
    txsbalance.push_back(createFvmTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<FvmTransaction> txs;
    txs.push_back(createFvmTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createFvmAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<FvmTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createFvmTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
