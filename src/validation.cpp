...
#if defined(NDEBUG)
# error "FiroVM cannot be compiled without assertions."
#endif
...
std::unique_ptr<FvmState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = fvmDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = fvmDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        FvmTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractFvmTX resultConverter;
        if(!converter.extractionFvmTransactions(resultConverter)){
        ...
        std::vector<EthTransactionParams> fvmETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(FvmTransaction fvmTransaction : fvmTransactions){
            sumGas += fvmTransaction.gas() * fvmTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, fvmTransaction.gasPrice());
            } else {
                txMinGasPrice = fvmTransaction.gasPrice();
            }
            VersionVM v = fvmTransaction.getVersion();
            if(v.format!=0)
            ...
            if(fvmTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(fvmTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(fvmTransaction.gas() > UINT32_MAX)
            ...
            if(v.rootVM!=0 && (uint64_t)fvmTransaction.gasPrice() < minGasPrice)
...
bool ReadBlockFromDisk(CBlock& block, const FlatFilePos& pos, const Consensus::Params& consensusParams)
{
    ...
    if(!CheckBlockHeader(block, consensusParams)){
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    FvmTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = FvmTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = FvmTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<FvmTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
bool CheckReward(const CBlock& block, BlockValidationState& state, int nHeight, const Consensus::Params& consensusParams, CAmount nFees, CAmount gasRefunds, const std::vector<CTxOut>& vouts)
{
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path fvmDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(fvmDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(fvmDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(FvmTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, FvmTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool ByteCodeExec::processingResults(ByteCodeExecResult& resultBCE){
    ...
    for(size_t i = 0; i < result.size(); i++){
        ...
        if(result[i].execRes.excepted != dev::eth::TransactionException::None){
        	...
        	if(!(result[i].execRes.excepted == dev::eth::TransactionException::RevertInstruction)){
        	    resultBCE.usedGas += gasUsed;
        	}
        }

        if(result[i].execRes.excepted == dev::eth::TransactionException::None || (result[i].execRes.excepted == dev::eth::TransactionException::RevertInstruction)){
...
bool FvmTxConverter::extractionFvmTransactions(ExtractFvmTX& fvmtx){
    ...
    std::vector<FvmTransaction> resultTX;
    ...
    fvmtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool FvmTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool FvmTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
FvmTransaction FvmTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    FvmTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = FvmTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = FvmTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t FvmTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    FvmDGP fvmDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setFvmSchedule(fvmDGP.getGasSchedule(pindex->nHeight));
    uint32_t sizeBlockDGP = fvmDGP.getBlockSize(pindex->nHeight);
    uint64_t minGasPrice = fvmDGP.getMinGasPrice(pindex->nHeight);
    uint64_t blockGasLimit = fvmDGP.getBlockGasLimit(pindex->nHeight);
    dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
    dgpMaxBlockSize = 80000000;
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize)
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        // Calculate burn address to effect supply
        for (const auto& tx_out : tx.vout) {
            CTxDestination destination = DecodeDestination("TDk19wPKYq91i18qmY6U9FeTdTxwPeSveo");
            if(tx_out.scriptPubKey == GetScriptForDestination(destination))
            {
                nBurn += tx_out.nValue;
            }
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            FvmTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractFvmTX resultConvertFvmTX;
            if(!convert.extractionFvmTransactions(resultConvertFvmTX)){
            ...
            if(!CheckMinGasPrice(resultConvertFvmTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertFvmTX.first, blockGasLimit, pindex->pprev);
            ...
            for(FvmTransaction& qtx : resultConvertFvmTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertFvmTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertFvmTX.first[k].from(),
                        resultConvertFvmTX.first[k].to(),
                        ...
                        resultConvertFvmTX.first[k].getNVout(),
    ...
    if(!CheckReward(block, state, pindex->nHeight, chainparams.GetConsensus(), nFees, gasRefunds, checkVouts))
    ...
    // Calculate Existing Money Supply
    uint64_t coinbaseValue = block.vtx[0]->vout[0].nValue - nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + coinbaseValue - nBurn;
    ...
    do {
        ...
        LimitValidationInterfaceQueue();
        {
            ...
            do {
                ...
                uint256 pblockGetHash;
                if(pblock != nullptr){
                    pblockGetHash = pblock->GetHash();
                }
                uint256 pindexMostWorkGetBlockHash = pindexMostWork->GetBlockHash();
                if (!ActivateBestChainStep(state, chainparams, pindexMostWork, pblock && pblockGetHash == pindexMostWorkGetBlockHash ? pblock : nullBlockPtr, fInvalidFound, connectTrace)) {
...
{
    if(gArgs.IsArgSet("-validatekey")) {            
        std::string strPubKey = gArgs.GetArg("-validatekey", "");
        bool invalid;
        std::vector<unsigned char> vchPubKey = DecodeBase64(strPubKey.c_str(), &invalid);
        if (invalid) {
            LogPrintf("CheckBlockHeader(): validatekey has invalided base64\n");
            throw ("CheckBlockHeader(): validatekey has invalided base64");
        }
        CPubKey pubkey = CPubKey(vchPubKey.begin(), vchPubKey.end());
        // verify sig
        if(!pubkey.Verify(block.GetHashWithoutSign(), block.vchBlockSig)){
            LogPrintf("CheckBlockHeader(): verify signature failed\n");
            return false;
        }else{
            return true;
        }
    }else{
        LogPrintf("CheckBlockHeader(): validatekey does not provide\n");
        throw ("CheckBlockHeader(): validatekey does not provide");
    }    
    return false;
}

bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckMerkleRoot)
{
    ...
    if (!CheckBlockHeader(block, consensusParams))
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "signature-hash", "block signature verification failed");;
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    const Consensus::Params& consensusParams = params.GetConsensus();
    if(!CheckBlockHeader(block, consensusParams)){
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-signature", "incorrect signature for block");
    }
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!CheckBlockHeader(block, chainparams.GetConsensus()))
...
bool TestBlockValidity(BlockValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckMerkleRoot)
{
    AssertLockHeld(cs_main);
...
void CChainState::CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    while (pindex != nullptr) {
        ...
        assert(pindex->pprev == nullptr || pindex->nHeight > pindex->pprev->nHeight); // For every block except the genesis block, the nHeight must be larger than the parent's.
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        FvmTxConverter convert(tx, NULL, NULL, contractflags);
