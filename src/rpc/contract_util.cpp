...
UniValue transactionReceiptToJSON(const FvmTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue CallToContract(const UniValue& params)
{
    ...
    if(params.size() >= 3){
        CTxDestination fvmSenderAddress = DecodeDestination(params[2].get_str());
        if (IsValidDestination(fvmSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&fvmSenderAddress);
    ...
    for(size_t i = 0; i < list.size(); i++)
    {
        ...
        for(size_t i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                ToFvmAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].get_str().substr(24);
                ToFvmAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
