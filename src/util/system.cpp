...
const char * const BITCOIN_CONF_FILENAME = "fvm.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FVM
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FVM
    // Mac: ~/Library/Application Support/FVM
    // Unix: ~/.fvm
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FVM";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FVM";
#else
    // Unix
    return pathRet / ".fvm";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Firo Core") == std::string::npos) {
