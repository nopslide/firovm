 ...
 */
static const char* BITCOIN_PID_FILENAME = "firovm.pid";
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-min-tx-gas-price=<amt>", "Any contract execution with a gas price below this will not be included in a block (defaults to the value specified by the DGP)", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    gArgs.AddArg("-max-tx-gas-limit=<n>", "Any contract execution with a gas limit over this amount will not be included in a block (defaults to soft block gas limit)", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    gArgs.AddArg("-soft-block-gas-limit=<n>", "After this amount of gas is surpassed in a block, no more contract executions will be added to the block (defaults to consensus-critical maximum block gas limit)", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/nopslide/firovm>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateFvm");
...
bool AppInitMain(NodeContext& node)
{
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            fs::path fvmStateDir = GetDataDir() / "stateFvm";
            bool fStatus = fs::exists(fvmStateDir);
            const std::string dirFvm(fvmStateDir.string());
            ...
            pstorageresult.reset(new StorageResults(fvmStateDir.string()));
            if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setFvmSchedule(fvmDGP.getGasSchedule(::ChainActive().Height()));
