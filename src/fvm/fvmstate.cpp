...
FvmState::FvmState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = FvmState::openDB(_path + "/fvmDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

FvmState::FvmState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute FvmState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, FvmTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createFvmAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, FvmTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, FvmTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> FvmState::vins() const // temp
{
...
void FvmState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* FvmState::vin(dev::Address const& _a) const
{
    return const_cast<FvmState*>(this)->vin(_a);
}

Vin* FvmState::vin(dev::Address const& _addr)
{
...
void FvmState::kill(dev::Address _addr)
{
...
void FvmState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void FvmState::deleteAccounts(std::set<dev::Address>& addrs){
...
void FvmState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void FvmState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void FvmState::validateTransfersWithChangeLog(){
